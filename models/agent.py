...
from duet.map_nav_src.utils.logger import print_progress
from duet.map_nav_src.r2r.eval_utils import cal_dtw
from duet.map_nav_src.networks.graph_utils import GraphMap
from duet.map_nav_src.networks.model import VLNBert, Critic
...
class Seq2SeqAgent(BaseAgent):
    ...
    def __init__(self, args, env=None, model=None, dataloader=None, logger=None):
        super().__init__(env)
        self.args = args
        self.rank = args.rank

        self.default_gpu = True if args.rank == 0 else False

        # Build Models
        self.vln_model = VLNModel(args, lang_encoder=model, logger=logger)
