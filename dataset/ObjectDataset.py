...
from dataset.process_multi_data import (
    load_r2r_data, load_ade_soon_data,
...
ERROR_MARGIN = 3.0
# from duet.vision_object.r2r.eval_utils import cal_dtw, cal_cls
...
class ObjectDataset(torch_data.Dataset):
    def __init__(
            self,
            config,
            args,
    ...
    def eval_dis_item(self, traj, gt_item):
        scores = {}
        scan = gt_item['scan']
    ...
    def eval_metrics(self, preds, logger=None, data_type=None):
        ...
        for item in preds:
            ...
            if data_type == 'cvdn':
                ...
                gt_item = self.gt_trajs[instr_id]
                traj_scores = self.eval_dis_item(traj, gt_item)
