...
def rollout(
    ...
    # history_infos = {
    #     'txt': ["" for _ in range(len(instructions))],
    #     'vis': [[] for _ in range(len(instructions))],
    #     'prev_txt': ["" for _ in range(len(instructions))],
    #     'prev_vis': [[] for _ in range(len(instructions))],
    # }
    history = []
    hist_vis = []
    ...
    for t in range(max_action_len):
        ...
        with context():
            ...
            # Not compute loss
            # get history vision infos
            # for idx in range(len(a_t)):
            #     if a_t[idx] == -100:
            #         continue
            #     candidate_nums = nav_outs['fuse_embeds'].size(1)
            #     history_infos['prev_txt'][idx] += 'History: <hist>\n'
            #     history_infos['txt'][idx] += \
            #         '\nEnvironment: ' + \
            #         ''.join(['<cand>' for _ in range(candidate_nums)]) + \
            #         '\nAgent: <s>'
            #     history_infos['prev_vis'][idx].append(
            #         nav_outs['fuse_embeds'][idx][a_t[idx], None]
            #     )  # (1,768)
            #     history_infos['vis'][idx].append(
            #         nav_outs['fuse_embeds'][idx]
            #     )  # (n, 768)
