...
class GMapNavAgent(Seq2SeqAgent):
    ...
    def rollout(self, train_ml=None, train_rl=False, reset=True, do_backward=False):
        ...
        # import pdb;pdb.set_trace()
        if self.args.cand_loss and train_ml is not None:
            cnt_qa = [self.jj['{}_{}'.format(ob['scan'], ob['path_id'])] for ob in obs]
        ...
        for t in range(self.args.max_action_len):
            ...
            with context():
                ...
                nav_inputs['do_permute'] = (train_ml is not None)
                ...
                if train_ml is not None:
                    ...
                    # ========================
                    if self.args.cand_loss and self.feedback=='teacher': # cand loss
                        qas = []
                        pano_inputs = self._panorama_feature_variable(obs)
                        pano_embeds, pano_masks = self.vln_bert('panorama', pano_inputs)
                        ...
                        qa_loss = self.vln_bert('qa', nav_inputs)
                        qa_loss.backward()

                    # ========================
                    if self.args.hist_loss and self.feedback=='teacher' and flag: # gen loss
                        pano_inputs = self._panorama_feature_variable(obs)
                        pano_embeds, pano_masks = self.vln_bert('panorama', pano_inputs)
                        ...
                        sum_loss = self.vln_bert('sum', nav_inputs)
                        sum_loss.backward()
